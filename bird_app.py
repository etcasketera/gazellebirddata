...
@st.cache_data
def run_bulk_analysis(files, engine="BirdNET"):
    all_results = []
    if files:
    ...
    if files:
        ...
        for i, filename in enumerate(files):
            ...
            if engine == "BirdNET":
                ...
                records = analyzer.analyze(filename)
...
if __name__ == "__main__":
    ...
    engine = st.selectbox("Select Analysis Engine", ["BirdNET", "Google Perch"])

    if folder_path and os.path.exists(folder_path):
        results_filename = 'birdnet_results.csv' if engine == 'BirdNET' else 'perch_results.csv'
        ...
        if st.session_state.df is None:
            ...
            if col2.button("🚀 Start New Analysis"):
                ...
                if files:
                    df_results = run_bulk_analysis(files, engine=engine)
